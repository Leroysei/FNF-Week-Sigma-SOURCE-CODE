...
class Note extends FlxSprite
{
	...
	public static var swagWidth:Float = 112; // 160 * 0.7
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false)
	{
		...
		{
			PlayState.holdNoteHieght = Conductor.stepCrochet / 100 * (0.018 * PlayState.SONG.bpm * PlayState.SONG.speed * (PlayState.SONG.bpm * PlayState.SONG.bpm * .000050544
				+ PlayState.SONG.bpm * -.018736 + 2.202));
		}
		else
		{
		...
		switch (daStage)
		{
			case 'school' | 'schoolEvil':
				...
				updateHitbox();
			case 'flying-stage':
				frames = Paths.getSparrowAtlas('business/sigmaUI/NOTE_assets', 'sigma');
	...
	override function update(elapsed:Float)
	{
		...
		if (mustPress)
		{
			...
			{
				if (strumTime > Conductor.songPosition - Conductor.safeZoneOffset
					&& strumTime - Conductor.songPosition < 250 / (PlayState.SONG.bpm / 100))
				{
					canBeHit = true;
				}
				else
